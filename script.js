...
// Logika Custom Prompt PWA Install
let deferredPrompt;
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    if (!isDismissed && installContainer) {
        // Tampilkan banner instalasi kustom kita
...
if (installBtn) {
    installBtn.addEventListener('click', async () => {
        ...
        if (deferredPrompt) {
            ...
            // Tunggu user merespon prompt tersebut
            const { outcome } = await deferredPrompt.userChoice;
            console.log(`User response to the install prompt: ${outcome}`);
            // Kosongkan deferredPrompt karena hanya bisa digunakan sekali
            deferredPrompt = null;
        }
    });
}
...
if (closeInstallBtn) {
    closeInstallBtn.addEventListener('click', () => {
        // Sembunyikan banner
        installContainer.classList.add('hidden');
        // Ingat pilihan user untuk tidak menginstal sementara waktu
